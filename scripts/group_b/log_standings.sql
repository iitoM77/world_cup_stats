-- 1. Drop and recreate the Group_B table
...
-- 2. Insert Group B standings
WITH group_b AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'B'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'B'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'B'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'B'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_b)
      AND team_b IN (SELECT team FROM group_b)
...
INSERT INTO Group_B (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
