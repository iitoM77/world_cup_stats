-- 1. Drop and recreate the Group_H table
...
-- 2. Insert Group H standings
WITH group_h AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'H'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'H'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'H'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'H'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_h)
      AND team_b IN (SELECT team FROM group_h)
...
INSERT INTO Group_H (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
