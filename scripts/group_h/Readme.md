...
## Overview
The `Group_H` table stores the standings for Group H in the World Cup dataset.  
...
## Table Structure
The `Group_H` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'H'`.  
...
4. Inserts the aggregated results into the permanent `Group_H` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_H` table with standings.  
- Extendable: You can replicate this script for other groups (I–L) by changing the group filter in the CTE.
