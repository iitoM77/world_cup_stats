-- 1. Drop and recreate the Group_D table
...
-- 2. Insert Group D standings
WITH group_d AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'D'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'D'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'D'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'D'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_d)
      AND team_b IN (SELECT team FROM group_d)
...
INSERT INTO Group_D (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
