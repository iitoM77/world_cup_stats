...
## Overview
The `Group_D` table stores the standings for Group D in the World Cup dataset.  
...
## Table Structure
The `Group_D` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'D'`.  
...
4. Inserts the aggregated results into the permanent `Group_D` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_D` table with standings.  
- Extendable: You can replicate this script for other groups (E–L) by changing the group filter in the CTE.
