-- 1. Drop and recreate the Group_E table
...
-- 2. Insert Group E standings
WITH group_e AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'E'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'E'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'E'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'E'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_e)
      AND team_b IN (SELECT team FROM group_e)
...
INSERT INTO Group_E (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
