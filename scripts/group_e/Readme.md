...
## Overview
The `Group_E` table stores the standings for Group E in the World Cup dataset.  
...
## Table Structure
The `Group_E` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'E'`.  
...
4. Inserts the aggregated results into the permanent `Group_E` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_E` table with standings.  
- Extendable: You can replicate this script for other groups (F–L) by changing the group filter in the CTE.
