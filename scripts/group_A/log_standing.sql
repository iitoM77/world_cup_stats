-- 1. Drop and recreate the Group_A table
...
-- 2. Insert Group A standings
WITH group_a AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'A'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'A'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'A'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'A'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_a)
      AND team_b IN (SELECT team FROM group_a)
...
INSERT INTO Group_A (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
