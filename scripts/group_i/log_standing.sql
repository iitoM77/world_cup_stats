-- 1. Drop and recreate the Group_I table
...
-- 2. Insert Group I standings
WITH group_i AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'I'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'I'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'I'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'I'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_i)
      AND team_b IN (SELECT team FROM group_i)
...
INSERT INTO Group_I (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
