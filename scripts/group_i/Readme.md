...
## Overview
The `Group_I` table stores the standings for Group I in the World Cup dataset.  
...
## Table Structure
The `Group_I` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'I'`.  
...
4. Inserts the aggregated results into the permanent `Group_I` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_I` table with standings.  
- Extendable: You can replicate this script for other groups (J–L) by changing the group filter in the CTE.
