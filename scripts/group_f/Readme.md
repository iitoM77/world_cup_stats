...
## Overview
The `Group_F` table stores the standings for Group F in the World Cup dataset.  
...
## Table Structure
The `Group_F` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'F'`.  
...
4. Inserts the aggregated results into the permanent `Group_F` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_F` table with standings.  
- Extendable: You can replicate this script for other groups (G–L) by changing the group filter in the CTE.
