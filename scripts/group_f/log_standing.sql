-- 1. Drop and recreate the Group_F table
...
-- 2. Insert Group F standings
WITH group_f AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'F'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'F'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'F'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'F'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_f)
      AND team_b IN (SELECT team FROM group_f)
...
INSERT INTO Group_F (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
