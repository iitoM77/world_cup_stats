-- 1. Drop and recreate the Group_G table
...
-- 2. Insert Group G standings
WITH group_g AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'G'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'G'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'G'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'G'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_g)
      AND team_b IN (SELECT team FROM group_g)
...
INSERT INTO Group_G (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
