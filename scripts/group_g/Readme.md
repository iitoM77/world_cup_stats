...
## Overview
The `Group_G` table stores the standings for Group G in the World Cup dataset.  
...
## Table Structure
The `Group_G` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'G'`.  
...
4. Inserts the aggregated results into the permanent `Group_G` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_G` table with standings.  
- Extendable: You can replicate this script for other groups (H–L) by changing the group filter in the CTE.
