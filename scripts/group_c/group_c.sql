-- 1. Drop and recreate the Group_C table
...
-- 2. Insert Group C standings
WITH group_c AS (
    SELECT "Team 1" AS team FROM tournament_groups WHERE "Group" = 'C'
    UNION ALL
    SELECT "Team 2" FROM tournament_groups WHERE "Group" = 'C'
    UNION ALL
    SELECT "Team 3" FROM tournament_groups WHERE "Group" = 'C'
    UNION ALL
    SELECT "Team 4" FROM tournament_groups WHERE "Group" = 'C'
),
team_stats AS (
    SELECT
        team_a AS team,
    ...
    WHERE team_a IN (SELECT team FROM group_c)
      AND team_b IN (SELECT team FROM group_c)
...
INSERT INTO Group_C (team, matches_played, wins, draws, losses, goals_scored, goals_against, goal_difference, points)
SELECT
    team,
