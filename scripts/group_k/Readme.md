...
## Overview
The `Group_K` table stores the standings for Group K in the World Cup dataset.  
...
## Table Structure
The `Group_K` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'K'`.  
...
4. Inserts the aggregated results into the permanent `Group_K` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_K` table with standings.  
- Extendable: You can replicate this script for other groups (L and beyond) by changing the group filter in the CTE.
