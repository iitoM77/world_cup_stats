...
## Overview
The `Group_L` table stores the standings for Group L in the World Cup dataset.  
...
## Table Structure
The `Group_L` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'L'`.  
...
4. Inserts the aggregated results into the permanent `Group_L` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_L` table with standings.  
- Extendable: You can replicate this script for other groups by changing the group filter in the CTE.
