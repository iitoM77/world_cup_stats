...
## Overview
The `Group_J` table stores the standings for Group J in the World Cup dataset.  
...
## Table Structure
The `Group_J` table has the following columns:
...
## Script Logic
1. Extracts the four teams from `tournament_groups` where `Group = 'J'`.  
...
4. Inserts the aggregated results into the permanent `Group_J` table.

---

## Usage Notes
- Run this script after populating `group_stage_results` and `tournament_groups`.  
- The output is a permanent `Group_J` table with standings.  
- Extendable: You can replicate this script for other groups (K–L) by changing the group filter in the CTE.
